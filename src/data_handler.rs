...
impl ServerData {
    ...
    pub fn set_player_data(&mut self, player_name: &str, player_data: PlayerData) {
        let _ = self
            .player_data
            .insert(player_name.to_string(), player_data);
    }
    /// doesn't save for you
    pub fn set_online(&mut self, is_online: bool) {
        self.is_online = is_online;
    ...
    pub async fn fetch_slp(&self) -> Result<serde_json::Value, Error> {
        ...
        return match tokio::time::timeout(
            rcon_time_limit,
            pinger::ping(&self.server_address, self.server_port),
        )
        .await
        {
            Ok(result) => result,
    ...
    pub fn save(&self) -> Result<(), Error> {
        let json_str = serde_json::to_string(self).map_err(|e| Error {
        ...
        return save_result.map_err(|e| Error {
    ...
    fn read_from_path(path: &str) -> Result<Self, Error> {
        let file_content = read_to_string(path).map_err(|e| Error {
...
pub fn get_all_server_data(root_dir: &str) -> Result<HashMap<u64, ServerData>, Error> {
    ...
    let dir_items = read_dir(root_dir).map_err(|e| Error {
    ...
    for entry in dir_items.filter(|e| e.is_ok()) {
        ...
        if let Some(filename) = filename.to_str() {
            ...
            let Some(guild_id) = filename.strip_suffix(".json") else {
                eprintln!("filename strip fail: {filename:?}");
                continue;
            };
            let Ok(guild_id) = guild_id.parse::<u64>() else {
                eprintln!("guild_id parse fail: {guild_id:?}");
                continue;
            };
